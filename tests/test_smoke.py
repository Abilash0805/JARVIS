...
def test_specs_render():
    ...
    # Closing apps/windows and richer mouse control are available too.
    assert {"close_app", "close_window"} <= names
    assert {"click", "double_click", "right_click", "scroll",
            "get_screen_size"} <= names
